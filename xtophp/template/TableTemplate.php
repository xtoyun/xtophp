...
abstract class TableTemplate  extends Template{
            ...
            foreach ($source as $key => &$row) {
                ...
                if (!empty($this->getData('columnbtn'))) {
                    foreach ($this->getData('columnbtn') as $index => $btn) {
                        ...
                        if (!empty($btn['icon'])) {
                            $row['button'] .="<a href=\"$p_href\"><div class='$p_class'><i class='$p_icon'></i></div></a>";
                        }else{
    ...
    private function getDefault($row,$cs,$txt){
        foreach ($cs as $key => $value) {
            if(isset($row[$value['name']])){
                $txt=str_replace('$'.$value['name'], $row[$value['name']], $txt);
            }
