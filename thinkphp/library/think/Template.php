...
class Template
{
    ...
    public function fetch($template, $vars = [], $config = [])
    {
        ...
        if ($template) {
            $mdvalue=$template; //以下为了考虑每个模块都生成一个key
            if (!empty($this->data['controller'])) {
                $mdvalue=$mdvalue.$this->data['controller'];
            }
            if (!empty($this->data['module'])) {
                $mdvalue=$mdvalue.$this->data['module'];
            }
            if (!empty($this->data['action'])) {
                $mdvalue=$mdvalue.$this->data['action'];
            } 
            //修改www.xtophp.com@谢

            $cacheFile = $this->config['cache_path'] . $this->config['cache_prefix'] . md5($mdvalue) . '.' . ltrim($this->config['cache_suffix'], '.');
