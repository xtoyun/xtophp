...
class Api extends BaseController{
	...
	public function article_category_update(){
		if(request()->ispost()){
			...
			$dao->map="(catename='$catename' and cateid<>$id)"; 
			if($dao->update($info)){
				return Util::message('保存成功',true);
			}
		}
	...
	public function about_update(){
		if(request()->ispost()){
			$data=request()->post();
			$dao 	=\app\web\dao\AboutDao::instance();
			$dao->map['nid']=input('nid');
			if($dao->saveorupdate($data)){
				return Util::message('更新成功',true);
			}
		}
		return Util::message('更新失败',false);
	...
	public function article_create(){
		...
		if(request()->ispost()){
			$data=request()->post();
			$dao 	=\app\web\dao\ArticleDao::instance();
			if($dao->save($data)){
				return Util::message('添加成功',true);
			}
		}
	...
	public function article_index_delete(){
		if(request()->ispost()){
			$id 	=input('id');
			$dao 	=\app\web\dao\ArticleDao::instance();
			if($dao->delete($id)){
