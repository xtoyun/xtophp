...
class Nav extends BaseController{
	...
	public function clink(){
		$p_nav=[];
		foreach (WebNav::where(null)->select() as $key => $value) {
	...
	public function get_model(){
		$id=input('model_id');
		return json(WebModel::find($id)->toArray());
	}
	...
	public function create(){
		...
		$models=[];
		$m_models=WebModel::where(null)->select();
  ...
  //           });  
	...
	public function edit(){ 
		$id=input('id'); 
		$nav_item=WebNav::find($id)->toArray();
		...
		$p_nav=[];
		foreach (WebNav::where("nid<>$id")->select() as $key => $value) {
			$p_nav[$value['title']]=$value['nid'];
		}
