...
class Pcategory extends BaseController{
	...
	public function index(){ 
		$list=WebProductCategory::where(null)->order('cateid desc')->paginate(10);
	...
	public function create_post(){
		if(request()->ispost()){
			...
			$category = new WebProductCategory();
	...
	public function edit(){
		$id=input('id'); 
		$category_item = WebProductCategory::find($id)->toArray();
	...
	public function edit_post(){
        if(request()->ispost()){
 		...
 		$category_item = WebProductCategory::find($cateid);
 		if ($category_item) {
	...
	public function delete_post(){
		if (request()->ispost()) {
			$cateid = input('id');
			$category_item = WebProductCategory::get($cateid,'sublist');
			//当存在上下级关系的时候所有删除失败
