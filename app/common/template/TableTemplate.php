...
abstract class TableTemplate  extends Template{
    ...
    public function setQuickSearchs($columns = [])
    {
        if (!empty($columns)) {
            foreach ($columns as $column) {
                call_user_func_array([$this, 'setQuickSearch'], $column);
            }
        }
        return $this;
    }

    public function setForm($template){
        $this->setDataList('forms',$template);
        return $this;
    }

     public function setForms($template){
        $this->setData('forms',$template);
        return $this;
    }
    ...
    public function setSearchs($columns = [])
    {
        if (!empty($columns)) {
            foreach ($columns as $column) {
                call_user_func_array([$this, 'setSearch'], $column);
            }
        }
        return $this;
    }

    public function setSearch($name = '',$default='',$title='',$class='', $vals = []){
        $result = [
                    'name'     => $name,
                    'title'     => $title,
                    'default'   => $default,
        ...
        $args   = array_slice(func_get_args(), 4);//生成数组为6
        $result = array_merge($result, $args);  
        $this->setDataList('searchs',$result);
        return $this;   
    }
